...
export async function handleMainCommand(url: string, options: MainOptions): Promise<void> {
  ...
  for (const content of contents) {
    ...
    try {
      ...
      platformSpinner.fail(`${content.platform} (${content.language}) - ${(error as Error).message}`);
      failCount++;
    }
  }
