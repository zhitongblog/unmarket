...
export interface PlatformRegistrationConfig {
  name: string;
  registerUrl: string;
  loginUrl?: string;  // For login instead of register
  googleOAuth?: {
    supported: boolean;
    buttonSelector?: string;  // Selector for "Sign in with Google" button
  };
  selectors: {
    emailField: string;
...
const PLATFORM_CONFIGS: Record<string, PlatformRegistrationConfig> = {
...
export class AccountRegistrar {
   ...
   */
  async register(platform: string): Promise<RegistrationResult> {
    ...
    try {
      ...
      if (config.requirements?.captchaExpected || config.requirements?.phoneRequired) {
        ...
        await this.unzoo.waitForUser(
          `Please complete the verification for ${config.name} (CAPTCHA/phone)`,
          120000
        );
      } else {
   ...
   */
  private async tryGoogleOAuth(config: PlatformRegistrationConfig): Promise<{ success: boolean }> {
    try {
   ...
   */
  async registerAll(platforms: string[]): Promise<RegistrationResult[]> {
   ...
   */
  private async checkForEmailVerification(): Promise<boolean> {
   ...
   */
  private async getVerificationCodeFromGmail(): Promise<string | null> {
    // Open Gmail
  ...
  private saveGmailState(email: string): void {
  ...
  private loadGmailState(): { email?: string; connectedAt?: string } {
    const db = getDatabase();
    const stmt = db.prepare('SELECT value, updated_at FROM session_state WHERE key = ?');
    const row = stmt.get('gmail_email') as { value: string; updated_at: string } | undefined;
